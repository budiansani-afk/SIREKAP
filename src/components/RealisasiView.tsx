...
export default function RealisasiView({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formNominal > maxAvailable) {
      const proceed = window.confirm(`Peringatan Defisit: Nominal pengeluaran ${formatRupiah(formNominal)} melebihi kuota Sisa Anggaran sub-kegiatan tersedia (${formatRupiah(maxAvailable)}). Apakah Anda yakin ingin memproses ini?`);
      if (!proceed) return;
    }
    ...
    <div className="space-y-6" id="realisasi-module-root">
      ...
      <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 text-xs" id="real-filters">
        <div className="flex flex-wrap gap-2.5">
          ...
          <div>
            <select value={selectedSubKeg} onChange={(e) => setSelectedSubKeg(e.target.value)} className="p-2 border border-slate-200 bg-white rounded-lg max-w-xs outline-blue-600">
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in" id="real-form-overlay">
          <div className="bg-white rounded-xl shadow-xl border border-slate-100 max-w-md w-full overflow-hidden" id="real-form-modal">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs max-h-[78vh] overflow-y-auto">
              ...
              {activeSubStatus && (
                <div className={`p-3 rounded-lg border text-[11px] ${activeSubStatus.sisa_tersedia < formNominal ? 'bg-red-50 border-red-100 text-red-900' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                  <p className="font-bold flex items-center gap-1">
                    <DollarSign size={13} />
                    Status Sisa Plafon Tersedia:
                  </p>
                  <ul className="list-disc list-inside mt-1 pl-0.5 space-y-0.5">
                    <li>Total Kontrak DPA Sub: <b>{formatRupiah(activeSubStatus.pagu)}</b></li>
                    <li>Sisa Anggaran Belanja Tersisa: <b className={`${activeSubStatus.sisa_tersedia < formNominal ? 'text-rose-700' : 'text-slate-900'}`}>{formatRupiah(activeSubStatus.sisa_tersedia)}</b></li>
                  </ul>
                  {activeSubStatus.sisa_tersedia < formNominal && (
                    <p className="mt-1.5 font-extrabold text-rose-700 flex items-center gap-1">
                      <AlertTriangle size={12} />
                      Overdraft! Nominal input melebihi Plafon DPA.
                    </p>
                  )}
                </div>
              )}
              ...
              <div>
                <label className="block text-slate-700 font-bold mb-1">Uraian / Keterangan Transaksi *</label>
                <input 
                  type="text"
                  placeholder="Contoh: Realisasi Jasa Pengukuran BPN dng Adendum SP2D"
                  value={formUraian}
